...
pos_prediction = zeros(1, 2);
vel_prediction = zeros(1, 2);
acc_prediction = zeros(1, 2);
...
grid on;
title({'Alpha-beta-gamma-delta: alfa = 0.75, beta = 0.8, gamma = 0.25, delta = 0.79','\newline','Data and threshold: 75'}, 'fontsize', font);
...
for k = 1 : c - 1
    ...
    % pasul 1: predictie
    pos_prediction(1,2) = pos_smooth + T * vel_smooth + (T^2/2) * acc_smooth + (T ^ 3 / 6) * j_smooth(1,2);
    vel_prediction(1,2) = vel_smooth + T * acc_smooth + (T ^2 / 2) * j_smooth(1,2);
    acc_prediction(1,2) = acc_smooth + T * j_smooth(1,2);
    
    % pasul 2: corectie
    pos_smooth = pos_prediction(1,2) + alfa * (pos_observed - pos_prediction(1,2));
    vel_smooth = vel_prediction(1,2) + (beta/T) * (pos_observed - pos_prediction(1,2));
    acc_smooth = acc_prediction(1,2) + (gama/(2 * T^2)) * (pos_observed - pos_prediction(1,2));
    j_smooth(1,2) = j_smooth(1,2) + (delta / (6 * T^3)) * (pos_observed - pos_prediction(1,2));

    
    % actualizeaza date
    pos_prediction(1,1) = pos_prediction(1,2);
    vel_prediction(1,1) = vel_prediction(1,2);
    acc_prediction(1,1) = acc_prediction(1,2);
    j_smooth(1,1) = j_smooth(1,2);
    
    % copiaza in B valoarea iteratiei
    B(1, k) = A1(1, k);
    
    % copiaza in B valoarea estimata pentru comparare cu valoarea de prag
    B(2, k) = pos_prediction(1,2);
...
grid on;
title({'Alpha-beta-gamma-delta: alfa = 0.75, beta = 2, gamma = 2.5, delta = 0.79','\newline','Data and threshold: 75'}, 'fontsize', font);
